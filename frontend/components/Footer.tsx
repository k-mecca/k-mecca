...
const Footer = () => {
  ...
  const handleCompleteClick = async () => {
    try {
      await productPost(barcode, photos);
      setIsCompleted(true);
    } catch (error) {
      console.error(error);
    }
    ...
    <footer className="absolute right-0 bottom-0 left-0 flex flex-col gap-2 p-4">
      {!isCompleted && (
        <button
          ...
          className="bg-kmecca rounded-md px-5 py-4 font-semibold text-[#1E2939]">
          완료하기
        </button>
      )}
      ...
      <button
        ...
        className={`rounded-md px-5 py-4 font-semibold ${isCompleted ? "bg-kmecca text-[#1E2939]" : "bg-[#EDEDED] text-[#8E8E93]"}`}>
